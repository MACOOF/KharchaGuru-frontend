...
export function Newincome() {
  const [open, setOpen] = useState(false)
  const [isPending, setisPending] = useState<boolean>(false)
  const t = useTranslations('newIncome')
  const router = useRouter()
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[95vw] max-w-[425px] p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle className="text-center sm:text-left">
            {/* Using dangerouslySetInnerHTML to handle the span in the translation */}
            <div dangerouslySetInnerHTML={{ 
              __html: t('dialogTitle').replace('<span>', '<span class="text-green-500">').replace('</span>', '</span>') 
            }} />
          </DialogTitle>
        </DialogHeader>
